...
MIGRATIONS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/src/migrations"

usage() {
  cat <<'USAGE'
Usage: scripts/run_migrations.sh [up|down]

Applies or rolls back SQL migrations in src/migrations against the database specified by $DATABASE_URL.
...
case "$ACTION" in
  up)
    mapfile -t migration_files < <(ls "$MIGRATIONS_DIR"/*.sql | sort)
    ;;
  down)
    mapfile -t migration_files < <(ls "$MIGRATIONS_DIR"/*.sql | sort -r)
    ;;
  *)
